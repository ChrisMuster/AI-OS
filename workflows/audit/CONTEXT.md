...
- `workflows/personal-data-guard/scripts/run.py` [[workflows/personal-data-guard/scripts/CONTEXT]] - A full audit shells out to its `--check --json` command and merges the WARN/FAIL findings under a `personal-data` label. Decoupled via subprocess, not import; a missing or broken guard degrades to an INFO note.
- `workflows/ai-style-guard/scripts/run.py` [[workflows/ai-style-guard/scripts/CONTEXT]] - A full audit shells out to its `--check --json --base main` command and merges the WARN findings under an `ai-style` label. Decoupled via subprocess, not import; a missing or broken guard degrades to an INFO note.
