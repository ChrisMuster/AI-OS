...
## Purpose
The universal, AI-agnostic safety net: a git pre-commit hook that fires at commit time no matter who staged the change (any AI, or a manual commit). It runs two gates in order: a hard block on a commit that would put personal data into tracked files (rule B3, layer 2), then a warn-not-block doc-sync CONTEXT/LOG drift advisory that prints a loud block but lets the commit through (close-out is the deterministic gate for that). The advisory reports drift only: it keeps the guard's WARN findings and stays silent on any other severity. What it *records* is wider than what it prints - each fire appends the guard's `INFO` and `DEGRADED` counts to the gitignored fire-log - but nothing about that reaches the commit-time surface. It is the sole preventive layer for AIs without a native hook (e.g. Aider) and a backstop under every other AI.
...
## Outputs
- Exit 0 (commit proceeds) or exit 1 with the block-and-explain message (commit aborted for personal data). A doc-sync drift advisory prints to stderr but keeps exit 0 (commit proceeds); a doc-sync finding at any other severity, including DEGRADED, prints nothing at all. One `doc_sync_recording` line per fire is appended to the gitignored fire-log, silently. The human override is `git commit --no-verify`.
...
- 2026-08-11 - Guard-coverage stage 14: the gate this shim calls now appends one `doc_sync_recording` line per fire to the gitignored fire-log, carrying the doc-sync guard's `INFO` and `DEGRADED` counts. Recorded here because this is the surface a person watches, and the visible behaviour at commit time is deliberately unchanged: the advisory still prints WARN findings and nothing else, so the new record is silent. The shim itself is untouched (still `run.py --precommit`).
