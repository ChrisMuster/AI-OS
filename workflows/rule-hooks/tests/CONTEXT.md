...
## Contents
- test_rule_hooks.py - `workflows/rule-hooks/tests/test_rule_hooks.py` [[workflows/rule-hooks/tests/CONTEXT]] - The full suite (60 tests): per-rule block/allow cases via the in-process Context pipeline, adapter input-parsing tests, subprocess tests asserting the real output contracts (Claude exit 2 + stderr; Codex `hookSpecificOutput` JSON deny), `TestPrecommitDocSync` (the pre-commit doc-sync advisory: the pure `format_doc_sync_advisory` text, that a personal-data block short-circuits before the advisory, that a clean personal-data check runs the advisory and still allows the commit, and that a doc-sync guard crash is swallowed), and `TestCwdIndependence` (the hook launches `run.py` by absolute path, so evaluation must survive a drifted working directory: `run.py` invoked from a temp directory outside the repo still blocks a `.env` write and still allows a normal write, plus a config guard that the shipped `.claude/settings.json` hook commands anchor `run.py` to `$CLAUDE_PROJECT_DIR`). Destructive cases use safe synthetic strings only; the B3 detection email is assembled at runtime so the literal never appears in the committed source.
