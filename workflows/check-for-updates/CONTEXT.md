...
- Landscape mode is only as good as web-research coverage. If sources fail (for example SSL or rate-limit errors), the section reports the run as inconclusive rather than clean. Keyword matching is a coarse signal: it can miss a deprecation phrased unusually and can flag an unrelated mention of a keyword, which is why the section is advisory and lists its sources for verification.
