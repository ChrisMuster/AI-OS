...
## Contents
- run.py — `workflows/web-research/scripts/run.py` — Main CLI entry point for the workflow.

## Inputs
None directly. run.py receives its inputs as CLI flags.

## Outputs
None directly. run.py saves the research package to `workflows/web-research/outputs/` and appends to `workflows/web-research/LOG.md`.

## Steps
N/A. This is a scripts container, not a workflow itself.

## Dependencies
- `skills/web-research/scripts/research.py` — Core research engine called by run.py.
