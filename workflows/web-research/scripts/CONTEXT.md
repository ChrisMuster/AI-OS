...
- 2026-09-23 - `run.py`'s `main()` now reconfigures stdout to UTF-8 as its first statement (guarded by `hasattr`), as the project encoding rule requires of any script printing a report. This also covers the progress lines printed by the shared skill `skills/web-research/scripts/research.py` [[skills/web-research/scripts/CONTEXT]], which is a library with no entry point of its own, so non-ASCII source names and error text no longer depend on the Windows console code page. Part of the report-scripts stdout sweep.
