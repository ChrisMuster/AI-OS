...
## Purpose
Makes the two required load-bearing SKILL.md sections a checked obligation rather than discipline alone. It is a deterministic, read-only checker in the guard family (same shape as `encoding-guard`, `personal-data-guard`, `ai-style-guard` [[workflows/ai-style-guard/CONTEXT]], and `doc-sync-guard` [[workflows/doc-sync-guard/CONTEXT]]): a standalone CLI that the full audit and the close-out verifier consume. For every SKILL.md under `skills/` [[skills/CONTEXT]] and `workflows/` [[workflows/CONTEXT]], it verifies (1) a `## Hardening` section exists with all five required fields (`Allowed tool intent`, `Never`, `Approval-gated`, `Write boundaries`, `Verification / escape hatch`) present and non-empty, and (2) a non-empty `## Verification` section exists (the caller-facing statement of how to confirm the skill's output is correct, required by the schema and the Verification-discipline rule). Both checks report under the one `skill-hardening` label.
...
- tests/ - `workflows/skill-hardening-guard/tests/` [[workflows/skill-hardening-guard/tests/CONTEXT]] - 29 hermetic tests for the parser/checker (both required sections), skill discovery over a throwaway tree, a template-drift guard (both the Hardening field labels and the presence of the Verification section), and a real-guard subprocess smoke test.
...
## Steps
1. Scan every SKILL.md for a complete Hardening section and a non-empty Verification section (read-only):
   `python workflows/skill-hardening-guard/scripts/run.py --check [--json] [--strict]`
2. Review any WARN findings and fill in the flagged Hardening section/field or the missing Verification section (the guard reports the file and the gap; it never edits).
...
## Dependencies
- `AGENTS.md` [[AGENTS]] (root) - Defines the SKILL.md schema and the required Hardening section (five fields) and Verification section this guard enforces.
- `templates/` [[templates/CONTEXT]] - The SKILL.md.template whose Hardening field labels the checker keys on; if the template's labels change, the guard's `REQUIRED_FIELDS` must change with them. The template also carries the `## Verification` section the guard now requires.
...
## Known Issues
- The check enforces presence and shape (the five Hardening fields and the Verification section exist and are non-empty), not the correctness of the declared policy or the substance of the verification steps. A Hardening section describing the wrong blast radius, or a Verification section whose stated check is weak, still passes; correctness stays a human/AI review judgement.
- Both required sections report under the single `skill-hardening` label (the guard was not renamed to keep churn off a just-committed workflow). A close-out failure line therefore reads `[skill-hardening] ... missing \`## Verification\` section`; the message names the actual gap even though the label historically referenced only Hardening.
...
- 2026-07-10 - Extended to also enforce the required `## Verification` section (umbrella Bucket-1 child #7): `check_skill` now checks both load-bearing sections independently (Hardening's five fields and a non-empty Verification section), reporting both under the one `skill-hardening` label so the audit hook and close-out gate need no new wiring. `extract_hardening_section` was generalised to `extract_section(text, heading)` with a back-compat wrapper; the exact-heading match stops the `Verification / escape hatch` Hardening field from satisfying the section check. Guard tests grew 23 -> 28 (a `VerificationSectionTests` class); the audit/close-out mechanism was unchanged.
- 2026-07-10 - Codex review follow-up (child #7): corrected two stale Hardening-only wordings in `run.py` (the docstring "Reports" block and the argparse description now name the Verification section too), tightened the tree-walk test to assert the exact two missing-section findings, and added a template-drift test that the SKILL.md template still carries a `## Verification` section. Guard tests 28 -> 29; no behaviour change.
