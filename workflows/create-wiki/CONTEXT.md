...
- Page-preserving PDF extraction JSON in the operating system's temporary directory, and optionally (with `--text`) a page-delimited plain-text rendering beside it for reading the source directly. This working data is not committed and does not modify the source PDF.
...
5. For PDF ingestion, run `python workflows/create-wiki/scripts/extract_pdf.py <project-relative-pdf> --text`, read the page-delimited text file it reports, and follow the generated wiki's ingest workflow. Omit `--text` when only the structured JSON is wanted.
...
- 2026-07-08 - Cleaned em dashes from the durable content and log strings generated by scripts/run.py, so newly-scaffolded wikis and the workflow's log entries use hyphens. Generated-output style change only; see scripts/CONTEXT.md for detail.
- 2026-07-24 - PDF extraction can now emit readable page-delimited text directly (`--text`), so the ingest step no longer depends on a hand-written flatten script recreated per session. See scripts/CONTEXT.md for detail.
