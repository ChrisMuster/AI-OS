...
## Contents
- archive.py - `workflows/session-search/scripts/archive.py` [[workflows/session-search/scripts/CONTEXT]] - Shared write layer. Iterates all registered adapters to discover and parse session files, then writes normalised records to the archive. Called by lightweight hooks (via `--hook`) and by index.py before indexing.
...
- `archive.py` canonicalises known AI identity aliases before writing archive records, so descriptive labels emitted by an assistant resolve to the supported setup labels.
...
- 2026-09-23 - `archive.py`, `index.py`, `discover.py` and `scheduler.py` now reconfigure stdout to UTF-8 as the first statement of `main()` (guarded by `hasattr`), as the project encoding rule requires of any script printing a report; `discover.py` gained `import sys` for it. Before this, a Windows pipe received cp1252: scheduler.py and the MCP server both decode index.py's output as UTF-8, and an adapter warning naming a non-ASCII file could raise `UnicodeEncodeError`. In `archive.py --hook` the JSON answer to the harness is ASCII, so its bytes are unchanged. Part of the report-scripts stdout sweep.
