...
4. Respond in plain prose citing session title, date, and source. Do not infer beyond what the snippets contain.
...
- Cowork sessions may lag up to one hour behind due to the scheduled-task capture window.
- Results are limited to sessions that have been indexed; run index.py if results seem stale.
