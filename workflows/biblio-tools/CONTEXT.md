...
## Purpose
MCP (Model Context Protocol) server that exposes Book Dragon project scripts as typed, callable tools. Provides a universal tool layer for any AI that supports MCP — typed parameters, cross-platform execution, and structured output. The underlying scripts are unchanged and still work via direct shell commands for AIs without MCP support.
...
2. Register the server in the AI's MCP configuration (see `AGENT-SETUP.md` for per-AI instructions; for Claude, the registration is in `.mcp.json` at the project root).
...
- Python `mcp` package (>= 1.0.0, requires Python 3.10+) — The MCP SDK providing FastMCP.
