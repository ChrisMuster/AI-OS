...
## Contents
- server.py - `workflows/biblio-tools/scripts/server.py` [[workflows/biblio-tools/scripts/CONTEXT]] - The FastMCP server defining all ten tools and their typed parameters, including the knowledge-graph build and query dispatcher assembled by the pure, unit-testable `build_kg_query_argv` helper. Its `mcp` import carries a `# runtime-guard: launched via launch.py` marker (below).
- verify.py - `workflows/biblio-tools/scripts/verify.py` [[workflows/biblio-tools/scripts/CONTEXT]] - Setup verification script (doctor pattern). Checks environment configuration, shared PDF ingestion, and the configured MCP server for any supported AI. Standard library only. Holds the project Python floor as `PYTHON_FLOOR` and judges both the Python running it and the project `.venv` interpreter against it via `floor_result`: below is a FAIL, above is a WARN tagged `"surface": true` (shown as `[SURFACE]` in plain text and counted in the JSON summary) so that session startup reports it, since the floor is only tested while it is the version the project runs.
...
- mcp_smoke.py - `workflows/biblio-tools/scripts/mcp_smoke.py` [[workflows/biblio-tools/scripts/CONTEXT]] - Connects to a configured stdio server, completes the MCP handshake, confirms the ten-tool inventory, calls read-only tools, checks input/path rejection, and verifies knowledge-graph query wiring using saved-index stats when available with a lightweight no-index fallback. Carries a `# runtime-guard: launched via verify.py/lifecycle_check.py under the project .venv` marker (below).
...
- setup.py - `workflows/biblio-tools/scripts/setup.py` [[workflows/biblio-tools/scripts/CONTEXT]] - Creates or repairs the canonical `.venv`, installs the root dependency manifest, and verifies package availability and the `.venv` interpreter version (one probe, `runtime_probe`). A `.venv` below its `PYTHON_FLOOR` fails `--check` and is refused for repair, with instructions to delete and recreate it; the script never deletes it.
...
## Revision History
Earlier history archived to LOG.md on 2026-07-05.
...
- 2026-09-23 - Python floor raised from 3.9 to 3.13. verify.py gained `PYTHON_FLOOR` and its version check now FAILs below the floor and WARNs above it rather than passing anything newer, and its MCP-package check lost the below-3.10 branch. setup.py gates on 3.13 and always checks `mcp`, which is no longer conditional. server.py lost two tool-docstring notes about running on 3.9; they were MCP tool descriptions, so a restarted server shows the shorter text.
- 2026-09-23 - Floor-review repairs (PF1, PF2). verify.py: the floor judgement moved into `floor_result`, whose above-floor WARN carries `"surface": true`; new `check_runtime_python_version` applies it to the `.venv` interpreter; `_venv_python` factored out of `_mcp_python`; `summarise` gives the JSON summary a `surface` count, and `format_results` marks surfaced results `[SURFACE]`. setup.py: `package_status` became `runtime_probe`, which also returns the `.venv` Python version; `--check` fails below the floor and `setup()` refuses to repair a below-floor `.venv`; the floor is now a named `PYTHON_FLOOR` rather than a literal.
- 2026-09-23 - verify.py's `main()` now reconfigures stdout to UTF-8 before printing, as the project encoding rule requires of any script printing a report. Without it a Windows pipe received cp1252 and the plain-text report's dashes arrived as replacement characters; the `--json` output was unaffected, being ASCII-escaped.
