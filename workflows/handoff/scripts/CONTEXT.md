...
- gather.py - deterministic signal readers (branch, `git status`, `git diff
  ...
  packet format it requires is deliberately minimal: an `##` section naming what is
  open and one naming what is addressed, matched on their first word against a
  closed synonym set (and skipped altogether where the heading contains the word
  "question"), with two sections of one kind merging so a packet can split its
  findings by class, and every other section free-form and ignored so a reviewer can
  lay a packet
  out however the round needs. Findings are counted as top-level bullets or `###`
  sub-headings, and a label is optional. There is now exactly one canonical label
  form, `R<round>-<finding>` or a bare `<LETTERS><number>`, stated in
  `memory/review_process.md`; the reader takes a label whole or not at all, so an
  attached suffix it cannot accept makes the item unlabelled rather than
  truncating it to a shorter label, and it reports any label outside the canonical
  form (`labels_noncanonical`) by name in the packet. A complaint never changes a
  count: a label it can still read is still used. Also the packet builder.
  Writes nothing.
...
- 2026-08-28 - Made the packet-format paragraph in Contents state what `review_packets` actually accepts, after the governing rule in `memory/review_process.md` was checked against this module and found to overstate it. The paragraph now says the two sections must be `##` headings, that the synonym set is closed rather than open-ended, that a heading containing the word "question" anywhere is skipped rather than only `Open questions`, and that two sections of one kind merge into a single count, which is the property that lets a packet split its findings into plan-readiness and review-process without inflating the number. Documentation-only; the reader is unchanged and the suite stays at 50 tests.
- 2026-09-18 - `_FINDING_LABEL` now keeps an attached `-N` or `.N` group as part of a finding's label. It stopped at the first run of digits, so a round labelling its findings `R13-1`, `R13-2` gave them all the label `R13`, and `_resolve_items`, which de-duplicates labelled items on purpose, then collapsed them into one: a packet of twelve addressed findings reported one, and the same shape would have rendered nine open findings as "1 open". That is the failure this reader exists to prevent, arriving through the label rather than through the section heading, which is the route the 2026-08-28 change closed. The dash must be attached, so `R1 - a description` still labels `R1` and no finding's prose is absorbed. The suite gained `TestLabelsCarryingARoundNumber` (50 to 54 tests), whose regression control is built from the packet that miscounted rather than from the new pattern, and the fix was mutation-tested: restoring the old pattern fires all four of the new tests and a no-op control fires none.
- 2026-09-18 - Closed the same collapse through the one route the entry above left open (R14-3, from the round-14 review), and added the standard that stops it recurring. `_FINDING_LABEL` still ended on `\b`, so where a suffix it could not accept was attached the pattern gave ground and matched whatever shorter prefix did end on a word boundary: `R13-1a` and `R13-2a` both became `R13`, and de-duplication merged two findings into one. The trailing position now refuses every character a label could have continued with, so an unacceptable suffix fails the match outright and the item reads as unlabelled; unlabelled items are counted and never de-duplicated, so the failure direction is now a lost label rather than a lost finding. The user's diagnosis went further than the defect and is the more durable half: an AI should not be inventing a label scheme per round, so there is now one canonical form and no other, written into `memory/review_process.md`, and `_read_label` reports any label outside it (`labels_noncanonical`) in the packet by name rather than tolerating it silently. Counting deliberately never regresses over a complaint, because a formatting fault must not cost a finding, and the complaint is resolved through the same headings-win rule as the count so it can never name an item the count discarded. Suite 54 to 66.
