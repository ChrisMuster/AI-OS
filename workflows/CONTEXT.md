...
- Encoding Guard - `workflows/encoding-guard/` [[workflows/encoding-guard/CONTEXT]] - Checks the project for encoding problems (invalid UTF-8, mojibake, BOMs, CR line endings where the project requires LF, text-mode subprocess calls with no explicit encoding, and text-mode writes with no explicit newline) and repairs damaged files to clean UTF-8, normalising a line-endings-only fault without touching anything else; it checks both the files that are already wrong and the code that would write them wrong again, and the full audit runs the check automatically.
