...
## Contents
- `run.py` - the close-out verifier entry point. Runs the structural audit and link audit in-process, runs the selected workflow test suites as subprocesses, and returns one aggregate pass/fail result with an exit code. The structural-audit gate fails on any audit FAIL and, additionally, on any `doc-sync`-labelled, `skill-hardening`-labelled, or `encoding`-labelled WARN finding: all three are advisory WARN inside the audit (so the audit's own exit code is unchanged) but a hard fail at close-out, the deterministic "done means done" gate (doc-sync from plan R2-3 Option B; skill-hardening added for umbrella child #6, same treatment). doc-sync catches CONTEXT/LOG drift; skill-hardening catches a SKILL.md missing its Hardening section, one of that section's five required fields, or its Verification section; encoding catches a violation of the AGENTS.md text-I/O rule, such as a text-mode write with no explicit `newline=` or a file carrying CR line endings. All three sets of messages are listed under the audit gate in the report. Only WARN counts - a `doc-sync`, `skill-hardening`, or `encoding` finding at DEGRADED severity means a check scope was skipped, either because the guard could not run or because one file was unreadable; it is non-blocking and reported through the DEGRADED path, not as a hard fail. Runtime-related DEGRADED findings can be retried with `--repair`, which runs `setup.py` and re-runs the gates once; file-level DEGRADED findings need the file-specific message handled.
...
- 2026-08-04 - `encoding` added to `BLOCKING_LABELS` as the third blocking label, so an encoding WARN hard-fails the structural-audit gate rather than being reported and ignored. A one-line change, exactly as the map's comment promises: the gate, the detail line, and the report all derive from it. The prompt was 50 real violations of the AGENTS.md text-I/O rule sitting inside a `RESULT: PASS` for a day. Same contract as the other two labels: only WARN blocks, DEGRADED stays non-blocking, and the label's INFO findings stay advisory.
- 2026-08-05 - Wording cleanup only: DEGRADED descriptions now cover both runtime
