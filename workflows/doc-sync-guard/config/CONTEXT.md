...
- 2026-09-01 - Added the `**/*-BRIEF.md` row, closing a failure the suite had been carrying rather than a gap somebody predicted. `.gitignore` gained that line when review briefs were filed with the four other design-time patterns, and no row was added with it, so `test_every_gitignore_line_has_a_row` had been failing and naming the line ever since. The suite is the thing that found it: the row was written after `boundary.py`'s first live run reported the same two brief documents relying on the Category C default in the sync architecture's classification, and running this suite to check whether the same drift had reached here returned a red result rather than a clean one. That is the case for a mechanical check over a documentation rule, since the same omission had passed unnoticed in three separate files. `provenance` is `sweep-1` rather than `decision-13a`, and the distinction matters: sweep-1 is defined as the derivation "every non-comment `.gitignore` line", which is exactly what this row is, whereas `decision-13a` means a path no sweep could have found because it does not exist yet, which is false here and would also break the test pinning that set at eleven. The example is synthetic, per the rule in this file's header that a real name here is the leak the personal-data guard exists to stop.
- 2026-09-21 - Widened the rule-hooks fire-log row from a concrete path to a pattern, `workflows/rule-hooks/fire-log*.jsonl`, with a synthetic `example`, alongside the matching `.gitignore` change. The rule-hooks store was retired to a dated archive, and the existing `.gitignore` line named the live file exactly, so an archive under any other name would have been committable - which matters because the rows record commands carrying real filesystem paths. **Widening the existing row was chosen over adding a second one**, and the reason is this file's own schema: a new row needs a `provenance`, and neither available value fits. `sweep-1` is defined as "every non-comment `.gitignore` line", which a second row for the same line is not, and `decision-13a` means a path no sweep could have found because it does not exist yet, which is both false here and pinned at eleven rows by a test. Widening keeps one row for one `.gitignore` line, so provenance stays true as written, and the row simply changes kind: it gains the `example` that refusal 1 requires of a patterned row and that refusal 2 forbids on a concrete one. Row count unchanged.
- 2026-09-25 - Added the `workflows/review-orchestration/runs/` row, in the same change as the `.gitignore` line it answers: `assert-ignored`, `seed: true` (a script writes there), `doc_sync: E10` (a data store of run records, like `session-search/data/`), `provenance: sweep-1`, and a synthetic run ID in the example.
