...
- 2026-09-23 - `run.py`'s `main()` now reconfigures stdout to UTF-8 as its first statement (guarded by `hasattr`), as the project encoding rule requires of any script printing a report. Before this, a Windows console or pipe received cp1252, which the MCP server's UTF-8 decode of `run_settings_check` output would garble on any non-ASCII finding. Adds `import sys`. Part of the report-scripts stdout sweep.
