...
0. **Verify Python** - before running any step that depends on scripts, confirm Python 3.13 or later is available on this machine.
   ...
   If no working Python 3.13+ command is found, stop immediately and tell the user: Python 3.13 or later is required to run Book Dragon - install it from python.org or ask an AI assistant to walk through installation for their operating system. Do not proceed with any further steps until Python is confirmed. This check runs on every session and every machine, not just on a fresh clone.
...
6. **Setup verification and AI-specific maintenance:**
   ...
   c. **Setup verification** - use the Biblio Tools MCP `verify_setup` tool when it is available for the current AI. Otherwise, run `python workflows/biblio-tools/scripts/verify.py --ai "[your AI name]"` silently. If any check returns FAIL, tell the user in a single sentence what is missing and point them to `AGENT-SETUP.md` for remediation. If all checks pass or only return WARN, say nothing, with one exception: a result carrying `"surface": true` (marked `[SURFACE]` in plain-text output) must be shown, so tell the user its detail in a single sentence.
...
For work that changes infrastructure or lifecycle code, meaning any workflow script or test, any git hook, and the files deciding which hooks fire or what is published, also route the diff through a writer/reviewer pass before commit: one agent writes, then a second with fresh context (ideally a different model) reviews the diff for correctness, edge cases, security, and rule adherence, with explicit licence to fail the work. A reviewer with no stake in the writer's reasoning catches what the writer cannot see.
...
4. **Backlog check** - at the natural end of a task (whether or not close-out follows), perform these sub-steps:
   a. Before editing `memory/backlog.md`, run `python workflows/backlog-guard/scripts/run.py --snapshot --reason "<why>"` whenever the workflow exists. After editing it, run `python workflows/backlog-guard/scripts/run.py --check`; if the edit deliberately reduced a section's count, pass only the explicit count-drop allowance for the number deliberately removed. A deliberate reduction is any change the user directed: a move to the completed archive or Build Only When Needed, or an outright deletion because the user said to drop the item. An item the user asks to remove is deleted cleanly and does not have to be completed or rehomed first; record what was removed in `memory/LOG.md` so the deletion leaves a trace. If the check reports a missing section, unexpected count drop, or oversized backlog item, stop and recover from `memory/backlog-backups/` before continuing.
...
- **A verification claim is written after the check, never before.** When an entry will state that a check passed, write it in two passes: pass one records what changed and carries no verification sentence, the checks then run, and pass two appends the results with their real numbers. This is written as a sequence rather than as a principle because the fault it prevents happens while writing the entry, not while thinking about verification, and at that moment the strongest cue is the shape of the neighbouring entries, which all end in a "Verified: ..." clause waiting to be filled. The process actively pushes the other way and the loop must be recognised rather than resolved by guessing: `doc-sync-guard` requires a changed directory's LOG.md to have gained an entry, and close-out hard-fails on a `doc-sync` WARN, so close-out cannot pass until the entries exist, while the entry is meant to report the close-out result. Two passes is the way through. Writing the whole entry and expecting the checks to confirm it produces a claim that is true only if the prediction holds, and that stays false in the audit trail if a check fails and nobody re-reads it. The same applies to any count or rate quoted in CONTEXT.md prose.
...
- Every text-mode write must also pass `newline="\n"` explicitly. On Windows, text mode translates every `\n` to `\r\n` on the way out, so a write that only pins the encoding still produces a CRLF file. `open(..., encoding="utf-8", newline="\n")`, `Path.write_text(..., encoding="utf-8", newline="\n")` and `write_bytes` are all acceptable on the project's 3.13 floor.
...
- The `encoding-guard` workflow [[workflows/encoding-guard/CONTEXT]] enforces all of the above, line endings included, on both sides: it checks the *files* (a CR ending is a WARN, and `--fix` normalises it without touching anything else in the file) and the *writers* (a text-mode write with no explicit `newline=` is a WARN, checked independently of the `encoding=` rule so pinning one never hides the other; the write shapes it matches are builtin `open(...)`, `Path.open(...)`, and `.write_text(...)`, and a shape outside that list is reported clean rather than reported missing; calls are found on the parse tree rather than by matching call-shaped text, so a *definition* sharing one of those names is not read as a call; both arguments are detected as parsed keywords rather than as text in the call, and a call whose arguments are splatted (`*args`/`**kwargs`) is skipped as unreadable, since presence cannot be proved there). Both arguments are also checked for their **value**, not only their presence, because both clauses above are stated as values: `encoding=` must name UTF-8 in any of its spellings, and `newline=` must be `"\n"` or the `""` that `csv` requires, so `encoding=None`, `encoding="cp1252"` and `newline=None` are WARN rather than compliant. A value is judged by what the literal evaluates to rather than by how it was typed, so `newline="\x0a"` passes as a line feed and `newline=r"\n"` is a WARN for the backslash-and-`n` it actually is; a raw string is still a literal and is read like any other. What stays unreadable is a value that is not a plain literal at all (a variable, an expression, an f-string, or a non-string literal), which is left silent rather than guessed at, the same trade the guard makes for a computed mode. Run `python workflows/encoding-guard/scripts/run.py --check` to scan and `--fix` to repair; the full audit runs the check automatically. A WARN under the `encoding` label is also a close-out hard fail, since `encoding` is one of the three blocking labels in the close-out verifier [[workflows/close-out/CONTEXT]]: a text-I/O or line-ending violation fails the gate rather than being reported and ignored. The label's INFO findings (an `open()` with no explicit `encoding=`) stay advisory, and a DEGRADED finding is non-blocking; it means either that the guard could not run at all or that one `.py` file could not be read as Python, and in both cases it says a check did not happen rather than that something is wrong. The file check and the writer check are separate clauses: a claim that one passes is not a claim about the other.
...
These rules are also enforced preventively by the `rule-hooks` workflow [[workflows/rule-hooks/CONTEXT]]: a universal git pre-commit hook blocks any commit that would add personal data to tracked files (for every AI and manual commits), and on Claude Code and Codex a PreToolUse hook blocks an in-progress write of personal data into a committable file before it lands. rule-hooks also makes several other always/never rules deterministic rather than prose-only: it blocks AI writes to `.env`, shell redirection into `LOG.md` (use the Edit tool or append_log), dangerous shell commands (`rm -rf`, force-push, `reset --hard`, `clean -fd`, destructive SQL, and what a `find -exec` runs), a shell file-read where a dedicated tool applies (`cat`/`sed`/`find`/`grep`/`head`/`tail` naming a file that exists - use Read/Grep/Glob instead; a byte-level pattern such as `$'\r'` is allowed and logged, because no dedicated tool can do it), inline interpreter code that writes a markdown file or `.env` (use the Edit tool), a PowerShell here-string passed to the Bash tool (use a POSIX heredoc), and a command hidden inside a wrapper option's value (`env -S` and `env --split-string`, whose value GNU env splits into words and runs; write the command directly), and re-injects the permission gate at session start. **Every one of those reads the command through a purpose-built shell reader rather than by matching text**, so a wrapper (`sudo`, `env`, `xargs`), a shell keyword (`if`, `for`), a substitution (`$(...)`, backticks), a `sh -c` string, an `eval`, or an expanded heredoc body does not hide the command inside it. The reader models the shell rules those cases depend on; it is not a shell, and where its model and a real shell diverge the difference is recorded in the workflow's Known Issues rather than assumed away. That last block is the one case the reader is not asked to see through, and the boundary is deliberate: `env -S`'s value is the command rather than configuration, so recovering it would mean reimplementing GNU env's own string splitting, where every difference between our version and the real one is a fresh bypass of the shape being closed. It is refused instead, which is the same trade that ended the `find -delete` adjudication. Hooks guarantee execution where prose only asks; every hard block ships with the exact manual path, so a genuinely-wanted action stays a deliberate human act.
...
1. Verify Python is available and meets the minimum version.
   - On Windows: run `python --version`. If that fails or returns Python 2, try `py --version`.
   - On macOS / Linux: try `python3 --version` first; fall back to `python --version` if needed.
   The output must show Python 3.13 or later. If no working command is found, or the version is below 3.13, stop immediately and tell the user: Python 3.13 or later is required - install it from python.org or ask an AI assistant to walk through installation for their operating system. Do not proceed with any further steps until Python is confirmed.
