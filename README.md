...
- **Review Orchestration** - Replaces the manual relay between two AIs with a scripted run: both plan a task independently from one brief, the plans are merged, then one builds while the other reviews in rounds that repeat automatically, with the user assigning every role and keeping every git decision. Built one sub-stage at a time; so far only the run brief template and its checker exist (`run.py --check-brief`), which refuses a brief that is not ready to start a run. `workflows/review-orchestration/` `[in progress]`
