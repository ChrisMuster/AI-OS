...
**Python 3.13+** is the only system-level requirement. Run `python workflows/biblio-tools/scripts/setup.py` once to create Book Dragon's shared project environment and install all workflow packages. See `REQUIREMENTS.md` for full details and API key setup.
...
- **Encoding Guard** - Checks the project for encoding problems (invalid UTF-8, mojibake, byte-order marks, CR line endings where the project requires LF, text-mode subprocess calls with no explicit encoding, text-mode writes with no explicit newline, and either argument passed with a value the rule does not allow, such as `encoding="cp1252"` or `newline=None`) and repairs damaged files to clean UTF-8 with LF line endings, normalising a line-endings-only fault without touching anything else. It covers both halves of the problem: the files that are already wrong, and the code that would write them wrong again. The code half finds its calls on the parse tree rather than by matching call-shaped text, so a definition that merely shares one of the names is not mistaken for a call, and a Python file it cannot parse is reported as unchecked rather than scanned as raw text. Runs standalone via CLI and automatically as part of the full audit, where an `encoding` WARN is a close-out hard fail rather than an advisory finding, while an unchecked-file report stays non-blocking. `workflows/encoding-guard/` `[active]`
...
- **Rule Hooks** - Deterministic rule enforcement that moves load-bearing always/never rules out of prose into hooks that block a violation the moment an AI acts. Covers Claude Code and Codex (blocking: protect .env, no shell redirect into LOG.md, no dangerous bash, no personal data in committable files, no PowerShell here-string in a Bash command, no shell file-read where a dedicated tool applies, no inline interpreter code writing a markdown file or `.env`, and no command hidden inside a wrapper option's value such as `env -S`; plus one log-only trial rule) and ships a universal git pre-commit personal-data gate that protects every AI and manual commits, with a SessionStart reminder of the permission gate. Every rule reads the command through a purpose-built shell reader rather than a lexer, so a wrapper, a shell keyword, a substitution, a `sh -c` string or an expanded heredoc body does not hide the command inside it. The reader models the shell rules those cases turn on; it is not a shell, and the points where it deliberately diverges are recorded in the workflow's Known Issues. `workflows/rule-hooks/` `[active]`
...
- **Handoff** - Clean session-to-session transitions. A trigger phrase ("do the handoff") runs a deterministic gather pass (branch, working tree, recent commits, changed-directory logs, active backlog, session activity, and the open findings of any review round part-way through) and Biblio writes a structured, rolling `HANDOVER.md`. On the next session a startup recovery step reads any unread handoff (tracked by a seen-watermark) and opens with an informed greeting, so the new session knows where the last one left off while the user keeps the choice. No per-AI slash commands; it works identically on every AGENTS-reading AI. `workflows/handoff/` `[active]`
...
- **Backlog Guard** - Protects `memory/backlog.md` with exact-copy local snapshots, post-edit required-section/count checks, explicit allowances for deliberate archive moves, oversized-item detection, and named snapshot restore. `workflows/backlog-guard/` `[active]`
- **Doc-Sync Guard** - Read-only checker that catches CONTEXT.md / LOG.md drift: for every committable directory that changed at all - whether its content moved or its own CONTEXT.md was the only file to change - it verifies the directory's CONTEXT.md moved (a Revision History entry, Last modified matching the newest entry) and its gitignored LOG.md gained an entry (verified by mtime, since a gitignored file cannot be content-diffed), plus the coarse child add/remove parent-propagation case. For a CONTEXT-only directory the LOG half reports INFO rather than WARN, built but switched off pending a live measurement, since it is the one clause that cannot be replayed over history. It fires at three stopping points: a warn-not-block advisory in the git pre-commit hook, a section in the handoff packet, and a close-out hard fail at WARN severity only, with a DEGRADED finding (the check could not run) surfaced but non-blocking. Makes the AGENTS.md CONTEXT/LOG maintenance rule mechanical rather than discipline-dependent. It also owns the project's classified output inventory: one tracked row per gitignored path recording whether the path must be ignored or tracked and which documentation exception applies, read through a single loader so guard output destinations are proved ignored before anything writes to them. `workflows/doc-sync-guard/` `[active]`
- **Doc-Verify** - Read-only structural self-consistency checker for a long markdown document, passed as an argument rather than hardcoded: it verifies that every table's rows match its separator's cell count, that enumerated table rows and numbered section headings are contiguous and in order, that every `path:line` code citation resolves against the repository, and it lists references-by-distance ("two bullets down") for adjudication. It also checks character hygiene in binary mode - tabs, trailing whitespace, a final newline, and any codepoint above U+007F - which covers the gap between Encoding Guard (UTF-8 validity, mojibake, BOMs, and since 2026-08-04 CR line endings project-wide) and AI-Style Guard (typographic tells, but scoped to `git diff` content and so blind to gitignored files). Its CR check now overlaps Encoding Guard's rather than being the only one, and remains useful for checking a single named document that the project-wide walk exempts. It also carries an opt-in, config-driven sequence sweep that inventories a class of statement inside a document (every sentence relating one named subject to another), reported as candidates for adjudication rather than as verdicts; its config declares the members already known to be in the class and the sweep fails if it cannot find them, so a hand-written vocabulary cannot quietly report an undercount as a finished inventory. It also carries an opt-in cross-document reference resolver (`--xref`) that resolves every numbered section reference and every quoted section title against the headings of all files passed in one command, matching on the joined text so a reference wrapped across a line break is still seen; a companion flag marks a decision log or ledger whose outbound references are records rather than instructions, so its findings are demoted rather than reported as breaks every run. It exists because verifying a document by writing a fresh throwaway script each review round means each runner re-implements the definition, and a wrong definition produces a confident wrong number. `workflows/doc-verify/` `[active]`
- **Sync Architecture** - Verification for a design document set and the file classification it defines, and the single shared implementation of the selection that classification drives. It extracts an executable classification block from the design document and proves what that block actually selects against the real working tree, asserting positive controls (properties that must be true of the selection) as well as exclusion sweeps, so the suite cannot pass on an empty classification. It also checks the documents for a set of named defect classes: a superseded pattern spelling left standing, a second copy of the classification that can drift from the first, a missing review baseline, and any script path a document names that neither exists in the tree nor is declared as something a stage has still to create. It reports how far each document has drifted from the version last reviewed, as information rather than as a failure: that drift is the diff the next review reads, and a baseline is refreshed only once the reviewing agent has recorded its findings, rather than once those findings are fixed. The selection itself lives in one module that both the checks and the build call, because the same defect appears in code as well as prose: the check resolved the classification in one git context while the build resolved it in another, which left its safety assertion unable to fail. That module resolves the pathspecs the way the build does, subtracts the files the public repository already tracks, and asserts the invariant underneath both, that no file is both tracked and ignored. It also carries the build-authorisation gate: before a stage is authorised, each design document must be byte-identical to its review baseline, so the plan being built is provably the plan that was reviewed. That is the same baseline measurement the consistency check makes, at a severity that depends on the phase, because drift is the diff the next review reads while a plan is being planned and evidence that the document changed after its review at the moment a build starts. It exists because a plan was found to have been built from a version no baseline had ever held. Read-only except for its staging mode, which writes to the private repository's index only and takes `--dry-run`. Exists because a classification written as prose in two places had a settled decision reach one copy and not the other. `workflows/sync-architecture/` `[active]`
- **Skill-Hardening Guard** - Read-only checker that verifies every SKILL.md carries the two required load-bearing sections: a complete `## Hardening` section (the declarative safety envelope: five fields - allowed tool intent, never-do actions, approval-gated operations, write boundaries, and a verification / escape hatch) and a non-empty `## Verification` section (how a caller confirms the skill's output is correct). It validates presence and shape only, never the truth of the declared policy, keeping runtime enforcement out of scope so the check stays portable across every AGENTS-reading AI. Advisory in the full audit; a close-out hard fail at WARN severity only, with a DEGRADED finding (an unreadable SKILL.md) surfaced but non-blocking. `workflows/skill-hardening-guard/` `[active]`
- **Review Orchestration** - Replaces the manual relay between two AIs with a scripted run: both plan a task independently from one brief, the plans are merged, then one builds while the other reviews in rounds that repeat automatically, with the user assigning every role and keeping every git decision. Built one sub-stage at a time. The run brief template and its checker exist (`run.py --check-brief`), which refuses a brief that is not ready to start a run. The engine is under way: so far the run record folder, and the usage-limit readings that stop a run before it spends allowance needed for interactive work, with an optional requirements file for the two AI SDKs. `workflows/review-orchestration/` `[in progress]`
...
- **Archived** - Archive destination for finished project-level design-time documents that have no owning workflow; a plan belonging to a workflow is archived into that workflow's own `archived/` directory instead. Documents arrive only once the work they specify is finished or abandoned. Contents are personal and local-only. `archived/` `[active]`
